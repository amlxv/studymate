...
import { computed, defineProps, toRefs, useSlots } from "vue";
...
const { disclosureTitle, disclosureContent } = toRefs(useSlots());

const isDisclosureTitle = computed(() => disclosureTitle?.value);
const isDisclosureContent = computed(() => disclosureContent?.value);
</script>

<template>
    <div class="pb-4 transition-all">
        <div
        ...
        <Disclosure v-slot="{ open }" v-if="isDisclosureTitle">
            <DisclosureButton
