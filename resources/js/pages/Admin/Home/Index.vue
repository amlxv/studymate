...
const props: ComputedRef<unknown> = computed(() => page?.props);

const stats = ref([
    {
        name: "Total Users",
        stat: props?.value?.users ?? 0,
        icon: AcademicCapIcon,
    },
    {
        name: "Total Schedules",
        stat: props?.value?.schedules ?? 0,
        icon: RectangleStackIcon,
    },
    {
        name: "Total Courses",
        stat: props?.value?.courses ?? 0,
...
<template>
    <Layout>
        <SectionHeading title="Dashboard" description="Simple. But powerful.">
        </SectionHeading>
